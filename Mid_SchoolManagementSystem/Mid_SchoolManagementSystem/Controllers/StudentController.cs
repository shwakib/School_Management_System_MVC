    ...
    public class StudentController : Controller
    {
        ...
        public ActionResult CheckNotice()
        {
            if ((string)Session["user"] != null)
            {
                ...
                var studentinfo = (from student in data.student
                                  where
                                    student.studentid == studentid
                                  select
                                  
                                      student.sectionid).FirstOrDefault();
                Debug.WriteLine(studentinfo);


                coursenotice[] noticeinfo = data.coursenotice.Where(x => x.sectionid == studentinfo).ToArray();
                TempData["noticedetails"] = noticeinfo;
                return View();
            }
            return RedirectToAction("Login", "User");
        }
        ...
        public ActionResult ViewGrade()
        {
            if ((string)Session["user"] != null)
            {
                ...
                grade[] grades = data.grade.Where(x => x.studentname == studentid).ToArray();
                TempData["gradedetails"] = grades;
                // Debug.WriteLine(grades);
