...
namespace Mid_SchoolManagementSystem.Controllers
{
    public class TeacherController : Controller
    {
        ...
        public ActionResult CreateCourseNotice()
        {
            if ((string)Session["user"] != null)
            {
              ...
              //subject[] sub = data.subject.ToArray();
                ...
                //var id = (string)Session["user"];
                assignedcourse[] noticeinfo = data.assignedcourse.Where(x => x.teacherid == teacherid).ToArray();
                TempData["teacherinfo"] = noticeinfo;
        ...
        public ActionResult CreateCourseNotice(coursenotice notice)
        {
            if ((string)Session["user"] != null)
            {
                data.coursenotice.Add(notice);
                data.SaveChanges();
                return RedirectToAction("TeacherIndex");
            }
            return RedirectToAction("Login", "User");
